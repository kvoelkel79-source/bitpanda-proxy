...
🤖 ${aiPrognose.kurzanalyse.slice(0,100)}` : "";
    ...
    // ── Chancen-Scanner: bekannte Top-Symbole analysieren ────────────
    const chancenCrypto = ["BTC","ETH","SOL","BNB","XRP","ADA","AVAX","LINK","DOT","NEAR"];
    const chancenStocks  = ["AAPL","NVDA","MSFT","TSLA","META","AMD","INTC","MU","ASML","NFLX"];
    const allChancen = [
      ...chancenCrypto.map(s=>({symbol:s,type:"crypto"})),
      ...chancenStocks.map(s=>({symbol:s,type:"stock"}))
    ];
    const fearGreedC = await getFearGreed();
    const chancenResults = [];

    for(const pos of allChancen){
      try{
        const sym = pos.symbol;
        const type = pos.type;
        const sig2 = {fearGreed: fearGreedC};
        if(type==="crypto"){
          const [cryptoData,orderbook,ticker,stocktwits,reddit,movingAverages,fundingRate] = await Promise.all([
            getCryptoData(sym),getOrderbookPressure(sym),getBinanceTicker(sym),
            getStockTwitsSentiment(sym),getRedditMentions(sym),
          ...
          Object.assign(sig2,{cryptoData,orderbook,ticker,stocktwits,reddit,movingAverages,fundingRate});
        } else {
          const [yahooPrice,technical,insider,stocktwits,reddit,movingAverages] = await Promise.all([
            getStockPriceYahoo(sym),getTechnicalIndicators(sym,""),
            getInsiderData(sym),getStockTwitsSentiment(sym),getRedditMentions(sym),
            getMovingAverages(sym)
          ]);
          Object.assign(sig2,{yahooPrice,technical,insider,stocktwits,reddit,movingAverages});
        }
        const score2 = calcScore(sig2, type);
        if(score2.score >= 58){
          // KI nur für Top-Chancen (score >= 70)
          let aiPrognose2 = null;
          if(score2.score >= 70 && ANTHROPIC_KEY){
            aiPrognose2 = await getAIPrognose(sym, type, sig2, score2, ANTHROPIC_KEY);
          }
          chancenResults.push({
            symbol: sym, type,
            score: score2,
            aiPrognose: aiPrognose2,
            signals: sig2,
            longName: sig2.yahooPrice?.longName || sig2.yahooPrice?.shortName || sym,
            updatedAt: new Date().toISOString()
          });
        }
      } catch(e){ console.error(`[Monitor] Chancen-Fehler ${pos.symbol}:`, e.message); }
    }

    // Top-10 nach Score sortiert speichern
    lastSignals["__chancen__"] = {
      results: chancenResults.sort((a,b)=>b.score.score-a.score.score).slice(0,10),
      updatedAt: new Date().toISOString()
    };
    console.log(`[Monitor] Chancen-Scanner: ${chancenResults.length} Chancen gefunden.`);

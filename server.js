...
// ── Score Calculator (gewichtete Quellen) ────────────────────────────────────
// Gewichtung: RSI/MACD 30% | Kursveränderung 25% | StockTwits 20% | Fear&Greed 15% | 7-Tage-Trend 10%
// Bei fehlender Quelle werden Gewichte proportional auf verfügbare Quellen verteilt
function calcScore(signals, type) {
  const bullish = []; // positive Signale
  const bearish = []; // negative Signale
  
  // Verfügbare Quellen tracken für Gewichts-Neuverteilung
  let weights = { tech:0.30, price:0.25, sentiment:0.20, feargreed:0.15, trend:0.10 };
  let available = { tech: false, price: false, sentiment: false, feargreed: false, trend: false };
  
  // Gewichtete Punkte: +1 = voll bullish, -1 = voll bärisch für diese Kategorie
  let weightedScore = 0;

  // ── RSI/MACD (30%) ──────────────────────────────────────────────
  if(signals.technical && signals.technical.rsi !== null) {
    available.tech = true;
    const t = signals.technical;
    let techPoints = 0;
    if(t.rsiSignal === "oversold_bullish")  { techPoints += 0.7; bullish.push(`RSI ${t.rsi?.toFixed(0)} – überverkauft (Kaufsignal)`); }
    if(t.rsiSignal === "overbought_bearish"){ techPoints -= 0.7; bearish.push(`RSI ${t.rsi?.toFixed(0)} – überkauft (Verkaufssignal)`); }
    if(t.macdCrossover === "bullish")       { techPoints += 0.5; bullish.push("MACD Kaufsignal (bullisher Crossover)"); }
    if(t.macdCrossover === "bearish")       { techPoints -= 0.5; bearish.push("MACD Verkaufssignal (bärischer Crossover)"); }
    techPoints = Math.max(-1, Math.min(1, techPoints));
    weightedScore += techPoints * weights.tech;
  }

  // ── Kursveränderung heute (25%) ──────────────────────────────────
  const priceChange = signals.yahooPrice?.change24h || signals.ticker?.changePercent || null;
  if(priceChange !== null) {
    available.price = true;
    let pricePoints = 0;
    if(priceChange >= 5)       { pricePoints = 1.0;  bullish.push(`+${priceChange.toFixed(1)}% heute – starkes Momentum`); }
    else if(priceChange >= 2)  { pricePoints = 0.5;  bullish.push(`+${priceChange.toFixed(1)}% heute – positiver Trend`); }
    else if(priceChange <= -5) { pricePoints = -1.0; bearish.push(`${priceChange.toFixed(1)}% heute – starker Verkaufsdruck`); }
    else if(priceChange <= -2) { pricePoints = -0.5; bearish.push(`${priceChange.toFixed(1)}% heute – negativer Trend`); }
    weightedScore += pricePoints * weights.price;
  }
  ...
  if(signals.stocktwits) {
    ...
    let stPoints = 0;
    if(st.bullPct >= 70)      { stPoints = 1.0;  bullish.push(`StockTwits ${st.bullPct}% bullish – sehr positive Trader-Stimmung`); }
    else if(st.bullPct >= 55) { stPoints = 0.4;  bullish.push(`StockTwits ${st.bullPct}% bullish – leicht positive Stimmung`); }
    else if(st.bullPct <= 30) { stPoints = -1.0; bearish.push(`StockTwits nur ${st.bullPct}% bullish – negative Trader-Stimmung`); }
    else if(st.bullPct <= 45) { stPoints = -0.4; bearish.push(`StockTwits ${st.bullPct}% bullish – leicht negative Stimmung`); }
    weightedScore += stPoints * weights.sentiment;
  }

  // ── Fear & Greed (15%) ───────────────────────────────────────────
  if(signals.fearGreed) {
    available.feargreed = true;
    const fg = signals.fearGreed;
    let fgPoints = 0;
    if(fg.value >= 75)      { fgPoints = 1.0;  bullish.push(`Fear&Greed ${fg.value} (${fg.classification}) – extreme Gier`); }
    else if(fg.value >= 55) { fgPoints = 0.4;  bullish.push(`Fear&Greed ${fg.value} (${fg.classification}) – positive Stimmung`); }
    else if(fg.value <= 25) { fgPoints = -1.0; bearish.push(`Fear&Greed ${fg.value} (${fg.classification}) – extreme Angst`); }
    else if(fg.value <= 45) { fgPoints = -0.4; bearish.push(`Fear&Greed ${fg.value} (${fg.classification}) – Vorsicht geboten`); }
    weightedScore += fgPoints * weights.feargreed;
  }

  // ── 7-Tage Trend (10%) ───────────────────────────────────────────
  const change7d = signals.cryptoData?.change7d || null;
  if(change7d !== null) {
    available.trend = true;
    let trendPoints = 0;
    if(change7d >= 10)      { trendPoints = 1.0;  bullish.push(`+${change7d.toFixed(1)}% in 7 Tagen – starker Aufwärtstrend`); }
    else if(change7d >= 5)  { trendPoints = 0.5;  bullish.push(`+${change7d.toFixed(1)}% in 7 Tagen – positiver Trend`); }
    else if(change7d <= -10){ trendPoints = -1.0; bearish.push(`${change7d.toFixed(1)}% in 7 Tagen – starker Abwärtstrend`); }
    else if(change7d <= -5) { trendPoints = -0.5; bearish.push(`${change7d.toFixed(1)}% in 7 Tagen – negativer Trend`); }
    weightedScore += trendPoints * weights.trend;
  }

  // ── Gewichts-Neuverteilung bei fehlenden Quellen ─────────────────
  const totalWeight = Object.entries(available).reduce((sum, [k,v]) => sum + (v ? weights[k] : 0), 0);
  if(totalWeight > 0 && totalWeight < 1) {
    weightedScore = weightedScore / totalWeight; // normalisieren
  }

  // Konvertierung: -1..+1 → 0..100 (50 = neutral)
  const rawScore = 50 + (weightedScore * 50);
...
async function runBackgroundMonitor() {
  try {
    if(!BITPANDA_KEY || !ANTHROPIC_KEY) return;
    console.log("[Monitor] Starte Hintergrund-Analyse...");

    // Portfolio von Bitpanda laden
    const r = await bpFetch("/v1/asset-wallets", BITPANDA_KEY);
    const positions = parsePositions(r.data);
